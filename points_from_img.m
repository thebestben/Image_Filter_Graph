function data = points_from_img(img_name, smoothing, sorted) 
    % load image
    [img,~] = imread(img_name);
    img = img(:,:,1);
    ...
    % Outputs
    data.axes = [x_axis; y_axis];

    data.points.numel = num_points;
    data.points.coords = points_coord;

    data.points_int.numel = num_unique_int_points;
    data.points_int.coords = points_coord_int_unique;

    data.image = img_ex(shape_rows:img_rows+shape_rows, shape_cols:img_cols+shape_cols);
    
    data.points_img.coords = points_img_inrange;
